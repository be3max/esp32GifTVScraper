...
function gistUrl(config) {
  return `${GITHUB_API_BASE}/${config.gistId}`;
}

function authHeaders(config) {
  return { 'Authorization': `token ${config.githubToken}` };
}
...
async function readGist(config) {
  try {
    const response = await request(gistUrl(config), {
