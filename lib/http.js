...
function request(url, options = {}) {
  ...
  return _request(url, options, maxRedirects, 0);
}

function _request(url, options, maxRedirects, redirectCount) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(httpsOptions, (res) => {
      ...
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
        res.destroy();
        _request(res.headers.location, options, maxRedirects, redirectCount + 1)
          .then(resolve)
          .catch(reject);
        return;
      }
