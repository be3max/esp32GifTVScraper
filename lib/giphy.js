 ...
 */
async function searchTag(tag, config, retries = 3) {
  const offset = Math.floor(Math.random() * config.maxOffset);
  const url = `${GIPHY_API_BASE}?q=${encodeURIComponent(tag)}&limit=${config.fetchLimit}&offset=${offset}&api_key=${config.giphyApiKey}`;

  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      const response = await request(url, { timeout: config.requestTimeout });
      const data = parseJson(response);
      return data.data || [];
    } catch (err) {
      if (err.statusCode === 429 && attempt < retries) {
        const delay = Math.pow(2, attempt) * 1000 + Math.random() * 500;
        console.warn(`⚠ GIPHY rate limit on "${tag}", retrying in ${Math.round(delay)}ms (attempt ${attempt + 1}/${retries})`);
...
async function fetchAllTags(config) {
  ...
  try {
    ...
    // Batch tag fetches to avoid GIPHY rate limits
    for (let i = 0; i < shuffledTags.length; i += concurrency) {
      const batch = shuffledTags.slice(i, i + concurrency);
      const batchResults = await Promise.all(batch.map(tag => searchTag(tag, config)));
      results.push(...batchResults);
    }

    // Flatten and deduplicate by GIF ID
    const seenIds = new Set();
    const allGifs = results.flat().filter(gif => {
      if (seenIds.has(gif.id)) return false;
      seenIds.add(gif.id);
      return true;
    });
...
async function filterBySize(gifs, config, limit = config.targetCount) {
  ...
  for (let i = 0; i < gifs.length; i += concurrency) {
    ...
    validGifs.push(...results.filter(Boolean));
...
module.exports = { searchTag, fetchAllTags, filterBySize, shuffle };
